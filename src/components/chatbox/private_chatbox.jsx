...
class PrivateChatBox extends Component {
  ...
  handleMessageFromSocket() {
    ...
    this.socket.on('privateVideo', message => {
  ...
  async handleMessageToSocket(event) {
    ...
    if (event.keyCode === 13 && text) {
      ...
      event.target.value = '';
      this.socket.emit('privateVideo', message)
      ...
      <div id="private_chat">
        ...
        }
        {
          ...
          context.props.currentChatStore.messages.map((message, index) => {
              return <p id="chat_list" key={index}><span id="common3">{message.from} : </span> <span id="messagebrah">{message.text}</span> <span id="time">{ Functions.timestampToDate(message.timestamp) }</span> </p>
            }) 
            : "No Message Yet"
        }
